...
class ProfileView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_customer:
            context.update(
                {
                    ...
                    "recent_feeds": DataFeed.objects.filter(customer=user).order_by(
                        "-created_at"
                    )[:5],
            ...
            context.update(
                {
                    "managed_customers": User.objects.filter(role="customer").count()
                    if user.is_admin
                    else 0,
                    "recent_activity": AuditLog.objects.filter(user=user).order_by(
                        "-timestamp"
                    )[:10],
                    "created_products": user.products_created.count()
                    if hasattr(user, "products_created")
                    else 0,
                }
            )
            ...
            context.update(
                {
                    ...
                    "pricing_count": CustomerPricing.objects.filter(
                        customer=user
                    ).count(),
                    "recent_downloads": FeedGeneration.objects.filter(
                        feed__customer=user, status="completed"
