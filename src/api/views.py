...
from products.models import Product, CustomerPricing, ProductFitment
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = [
            'id', 'sku', 'name', 'description', 'brand_name', 'categories',
            'weight', 'dimensions', 'msrp', 'map_price', 'is_active',
...
class AssetSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Asset
        fields = [
            'id', 'title', 'description', 'asset_type', 'file_size',
            'file_url', 'thumbnail_url', 'is_active', 'is_public', 'created_at'
        ]
...
class FeedSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = DataFeed
        fields = [
            'id', 'name', 'description', 'feed_type', 'format',
            'customer_name', 'is_active', 'created_at'
        ]
...
class BulkProductUpdateView(APIView):
    ...
    def post(self, request):
        ...
        if not product_ids or not action:
            return Response(
                {'error': 'product_ids and action required'},
                status=400
            )
