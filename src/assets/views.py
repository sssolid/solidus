...
from .forms import (
    AssetForm, AssetUploadForm, ProductAssetForm, AssetCategoryForm,
    AssetCollectionForm, AssetSearchForm, AssetTagForm
)
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_customer and self.request.user.allowed_asset_categories:
            context['categories'] = AssetCategory.objects.filter(
                slug__in=self.request.user.allowed_asset_categories,
                is_active=True
            )
        else:
...
class AssetDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['related_assets'] = Asset.objects.filter(
            Q(categories__in=asset.categories.all()) |
            Q(tags__in=asset.tags.all()),
            is_active=True
    ...
    if request.user.is_customer:
        if request.user.allowed_asset_categories:
            if not asset.categories.filter(
                ...
                messages.error(request, 'You do not have permission to download this asset.')
...
class AssetListView(EmployeeRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = Asset.objects.select_related('created_by').prefetch_related('categories')
    ...
    def get_context_data(self, **kwargs):
        ...
        context['total_size'] = Asset.objects.aggregate(
...
class AssetCreateView(EmployeeRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f'Asset "{self.object.title}" created successfully.')
        return response
...
class AssetEditView(EmployeeRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        messages.success(self.request, f'Asset "{self.object.title}" updated successfully.')
        return response
...
class CategoryManagementView(EmployeeRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        ).order_by('parent', 'sort_order', 'name')
...
def bulk_tag_assets(request):
    ...
    if form.is_valid():
        ...
        return JsonResponse({
            'success': True,
            'message': f'Tags {action}ed for {len(assets)} assets'
        })
    else:
