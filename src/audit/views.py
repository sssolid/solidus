...
from .models import AuditLog, ModelSnapshot, BulkOperation
...
class SnapshotListView(AdminRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return ModelSnapshot.objects.select_related('created_by').order_by('-created_at')
...
def restore_snapshot(request, pk):
    ...
    try:
        ...
        messages.success(request, f'Snapshot restored successfully.')
...
def compare_snapshots(request, pk):
    ...
    snapshot1 = get_object_or_404(ModelSnapshot, pk=pk)
    ...
    try:
        snapshot2 = ModelSnapshot.objects.get(id=snapshot2_id)
...
class BulkOperationListView(AdminRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return BulkOperation.objects.select_related('started_by').order_by('-started_at')
...
class AuditReportsView(AdminRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['top_users'] = AuditLog.objects.values(
