...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--feed-id", type=int, help="Generate a specific feed by ID"
        )
        parser.add_argument(
            "--force",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        if feed_id:
            ...
            try:
                ...
                self.stdout.write(self.style.ERROR(f"Feed {feed_id} not found"))
        else:
    ...
    def generate_scheduled_feeds(self):
        ...
        feeds = DataFeed.objects.filter(is_active=True).exclude(frequency="manual")

        generated_count = 0

        for feed in feeds:
            _next_run = feed.get_next_run_time()
            ...
            if feed.frequency == "hourly":
                ...
                if (
                    not feed.last_generated
                    or (now - feed.last_generated).seconds >= 3600
                ):
                    should_run = True
                ...
                if not feed.last_generated or feed.last_generated.date() < now.date():
                    _scheduled_today = timezone.datetime.combine(
                ...
                if now.day == feed.schedule_day:
                    if (
                        not feed.last_generated
                        or feed.last_generated.month != now.month
                    ):
                        should_run = True
        ...
        if generated_count == 0:
            ...
            self.stdout.write(self.style.SUCCESS(f"Generated {generated_count} feeds"))
    ...
    def generate_feed(self, feed):
        ...
        try:
            ...
            generation = FeedGeneration.objects.create(feed=feed, status="generating")
            ...
            if result["success"]:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"Successfully generated feed: {feed.name}")
                )
            else:
