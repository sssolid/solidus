...
from .models import DataFeed, FeedGeneration, Subscription, DeliveryConfig
from .forms import (
    DataFeedForm, SubscriptionForm, DeliveryConfigForm,
    FeedGenerationForm, FeedSearchForm, BulkFeedUpdateForm,
    FeedFieldMappingForm
)
from .tasks import generate_feed_task
...
class MyFeedsView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        if self.request.user.is_customer:
            return DataFeed.objects.filter(
                customer=self.request.user
            ).prefetch_related('generations').order_by('-created_at')
        else:
            # Employees can see all feeds
            return DataFeed.objects.all().prefetch_related('generations').order_by('-created_at')
    ...
    def get_context_data(self, **kwargs):
        ...
        context['active_subscriptions'] = Subscription.objects.filter(
            customer=self.request.user,
            is_active=True
        ).count() if self.request.user.is_customer else 0
...
class FeedListView(EmployeeRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = DataFeed.objects.select_related('customer').prefetch_related('generations')
...
class FeedCreateView(EmployeeRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f'Feed "{self.object.name}" created successfully.')
        return response
...
class FeedDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['stats'] = {
            ...
            'successful_generations': feed.generations.filter(status='completed').count(),
...
class FeedEditView(UpdateView):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        messages.success(self.request, f'Feed "{self.object.name}" updated successfully.')
        return response
...
def generate_feed(request, pk):
    ...
    if feed.generations.filter(status='pending').exists():
        return JsonResponse({'error': 'Feed generation already in progress'}, status=400)
    ...
    try:
        ...
        generation.save()
        return JsonResponse({'error': f'Failed to start generation: {str(e)}'}, status=500)
...
class GenerationListView(EmployeeRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return FeedGeneration.objects.select_related(
            'feed', 'started_by'
        ).order_by('-started_at')
...
class SubscriptionListView(ListView):
    ...
    def get_queryset(self):
        queryset = Subscription.objects.select_related('feed', 'customer')
