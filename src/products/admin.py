...
from .models import (
    Brand, Category, Product, VehicleMake, VehicleModel,
    ProductFitment, CustomerPricing
)
...
class CategoryAdmin(admin.ModelAdmin):
    list_display = ['name', 'parent', 'slug', 'product_count', 'sort_order', 'is_active']
...
class ProductFitmentInline(admin.TabularInline):
    ...
    extra = 1
    fields = ['make', 'model', 'year_start', 'year_end', 'submodel', 'engine', 'position']
...
class ProductAdmin(admin.ModelAdmin):
    list_display = [
        'sku', 'name', 'brand', 'display_categories', 'msrp',
        'fitment_count', 'is_active', 'is_featured'
    ]
    list_filter = [
        'is_active', 'is_featured', 'brand', 'categories',
        'created_at', 'launch_date'
    ]
    search_fields = [
        'sku', 'name', 'short_description',
        'part_numbers', 'oem_numbers'
    ]
    readonly_fields = [
        'created_at', 'updated_at', 'created_by',
        'fitment_count', 'display_primary_image'
    ]
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': (
                'sku', 'name', 'brand', 'categories',
                'short_description', 'long_description'
            )
        }),
        ('Part Numbers', {
            'fields': ('part_numbers', 'oem_numbers'),
            'classes': ('collapse',)
        }),
        ('Specifications', {
            'fields': (
...
class ProductFitmentAdmin(admin.ModelAdmin):
    list_display = [
        'product_sku', 'make', 'model', 'year_range',
        'submodel', 'engine', 'position'
    ]
...
class CustomerPricingAdmin(admin.ModelAdmin):
    list_display = [
        'customer_name', 'product_sku', 'price',
        'discount_percent', 'validity_period', 'created_at'
    ]
    list_filter = ['created_at', 'valid_from', 'valid_until']
    search_fields = [
        'customer__username', 'customer__email',
        'customer__company_name', 'product__sku',
        'product__name'
    ]
    ...
    fieldsets = (
        ...
        ('Pricing', {
            'fields': ('price', 'discount_percent', 'notes')
        }),
        ('Validity', {
